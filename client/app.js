App = {
    ...
    loadEthereum: async() => {
        if (window.ethereum){
            ...
            console.log('intall ethereum or metamask or dsomething!!!!!!!u idiot!!!S')
        }
    },
    ...
    loadContracts : async () => {
        const responde = await fetch("TasksContract.json")
        const tasksContractJSON = await responde.json()
    ...
    renderTasks: async() => {
        ...
        for (let i = 1; i <= taskCounterNumber; i++) {
            ...
            let taskElement = `
            <div class="card bg-darkie mb-2">
            <div class="card-header d-flex justify-content-between align-items-center"> 
                <span>${taskTitle}</span>
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" ${taskDone && "checked"} 
                    data-id="${taskId}"
